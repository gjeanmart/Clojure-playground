...
* [`src/cljs/clojurescript_todo/`](src/cljs/clojurescript_todo/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/clojurescript_todo/core.cljs): contains the SPA entry point, `init`
* [`test/cljs/clojurescript_todo/`](test/cljs/clojurescript_todo/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
7. Clone this repo and open a terminal in the `clojurescript-todo` project root directory
...
in the [`src/clj/clojurescript_todo/`](src/clj/clojurescript_todo/) directory. CSS files are compiled
...
The `debug?` variable in [`config.cljs`](src/cljs/clojurescript_todo/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns clojurescript-todo.example
  (:require [clojurescript-todo.config :as config])
